...
def linear_probing_minibatch(
    ...
    # if dataset_name in ["ogbn-papers100M", "mag-scholar-f", "mag-scholar-c", "ogbn-arxiv", "ogbn-products"]:
    eval_loader = setup_eval_dataloder(
        "lc",
        graph,
        feats,
    ...
    train_emb, val_emb, test_emb = (
        embeddings[:num_train],
        embeddings[num_train : num_train + num_val],
        embeddings[num_train + num_val :],
    )
...
def node_classification_linear_probing(
    ...
    for epoch in epoch_iter:
        ...
        for batch_x, batch_label in train_loader:
            ...
            pred = encoder(None, batch_x)
            loss = criterion(pred, batch_label.squeeze().long())
            optimizer.zero_grad()
            loss.backward()
...
def finetune(
    ...
    num_classes = (
        max(max(train_lbls.max().item(), val_lbls.max().item()), test_lbls.max().item())
        + 1
    )
    ...
    train_loader = setup_finetune_dataloder(
        "lc", graph, feats, train_egs, train_lbls, batch_size=batch_size, shuffle=True
    )
    val_loader = setup_finetune_dataloder(
        "lc", graph, feats, val_egs, val_lbls, batch_size=batch_size, shuffle=False
    )
    test_loader = setup_finetune_dataloder(
        "lc", graph, feats, test_egs, test_lbls, batch_size=batch_size, shuffle=False
    )

    # optimizer = torch.optim.Adam(model.parameters(), lr=lr_f, weight_decay=weight_decay_f)

    optimizer = torch.optim.AdamW(
        model.parameters(), lr=lr_f, weight_decay=weight_decay_f
    )
    ...
    if use_scheduler and max_epoch_f > 0:
        ...
        scheduler = lambda epoch: (
            epoch / warmup_epochs
            if epoch < warmup_epochs
            else (
                1
                + np.cos(
                    (epoch - warmup_epochs) * np.pi / (max_epoch_f - warmup_epochs)
                )
            )
            * 0.5
        )
    ...
    def eval_with_lc(model, loader):
        ...
        with torch.no_grad():
            for batch in epoch_iter:
                ...
                pred_counts.append((prediction.argmax(1) == batch_lbls.squeeze()))
    ...
    def eval_full_prop(model, g, nfeat, val_nid, test_nid, batch_size, device):
        ...
        return accuracy(pred[val_nid], val_lbls.cpu()), accuracy(
            pred[test_nid], test_lbls.cpu()
        )
    ...
    for epoch in range(max_epoch_f):
        ...
        for batch_g, targets, batch_lbls, node_idx in epoch_iter:
            ...
            prediction = model(batch_g, x)
            prediction = prediction[targets]
            loss = criterion(prediction, batch_lbls.squeeze().long())
            ...
            epoch_iter.set_description(
                f"Finetuning | train_loss: {loss.item():.4f}, Memory: {show_occupied_memory():.2f} MB"
            )
        ...
        if not full_graph_forward:
            ...
            if epoch > 0 and epoch % 1 == 0:
                val_acc, _test_acc = eval_full_prop(
                    model, graph, feats, val_nid, test_nid, 10000, device
                )
                model = model.to(device)
        ...
        if not full_graph_forward:
            ...
            print(
                "Val Acc {:.4f}, Best Val Acc {:.4f} Test Acc {:.4f}".format(
                    val_acc, best_val_acc, test_acc
                )
            )
        ...
        if logger is not None:
            logger.log(metrics)
        print(
            f"# Finetuning - Epoch {epoch} | train_loss: {np.mean(losses):.4f}, ValAcc: {val_acc:.4f}, TestAcc: {test_acc:.4f}, Memory: {show_occupied_memory():.2f} MB"
        )
...
def linear_probing_full_batch(
    ...
    optimizer_f = torch.optim.Adam(
        encoder.parameters(), lr=lr_f, weight_decay=weight_decay_f
    )
    final_acc, estp_acc = _linear_probing_full_batch(
        encoder, graph, x, optimizer_f, max_epoch_f, device, mute
    )
    return final_acc, estp_acc


def _linear_probing_full_batch(
    ...
    print(f"[SHAPE CHECK] Feature shape: {x.shape}")
    print(f"[SHAPE CHECK] Raw Labels shape: {labels.shape}")
    ...
    for epoch in epoch_iter:
        model.train()
        out = model(graph, x)
        loss = criterion(out[train_mask], labels[train_mask].squeeze().long())
        optimizer.zero_grad()
        loss.backward()
        # torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=3)
        optimizer.step()
        ...
        with torch.no_grad():
            model.eval()
            pred = model(graph, x)
            val_acc = accuracy(pred[val_mask], labels[val_mask].squeeze())
            val_loss = criterion(pred[val_mask], labels[val_mask].squeeze().long())
            test_acc = accuracy(pred[test_mask], labels[test_mask].squeeze())
            test_loss = criterion(pred[test_mask], labels[test_mask].squeeze().long())
